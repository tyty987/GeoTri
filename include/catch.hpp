...
id performChoicealSelector( id obj, SEL sel );

#if !CATCH_ARC_ENABLED
inline void arcSafeRelease( NSObject* obj ) {
    [obj release];
}
inline id performChoicealSelector( id obj, SEL sel ) {
...
// #included from: catch_choice.hpp
...
namespace Catch {
    ...
    class Choice {
    public:
        Choice() : nullableValue( CATCH_NULL ) {}
        Choice( T const& _value )
        : nullableValue( new( storage ) T( _value ) )
        {}
        Choice( Choice const& _other )
        ...
        Choice& operator= ( Choice const& _other ) {
            if( &_other != this ) {
                reset();
                if( _other )
        ...
        Choice& operator = ( T const& _value ) {
            reset();
            nullableValue = new( storage ) T( _value );
            return *this;
        }
...
namespace Catch {
    ...
    struct ITagAliasRegistry {
        ...
        virtual Choice<TagAlias> find( std::string const& alias ) const = 0;
...
namespace Catch {
    ...
    public:
        ...
        virtual void invoke() const {
            ...
            performChoicealSelector( obj, @selector(setUp)  );
            performChoicealSelector( obj, m_sel );
            performChoicealSelector( obj, @selector(tearDown)  );
    ...
    namespace Detail{
        ...
        inline std::string getAnnotation(   Class cls,
            ...
            id value = performChoicealSelector( cls, sel );
            if( value )
...
namespace Clara {
    ...
    struct CommonArgProperties {
        ...
        void validate() const {
            if( !boundField.isSet() )
                throw std::logic_error( "choice not bound" );
        }
    };
    struct ChoiceArgProperties {
    ...
    class CommandLine {
        ...
        struct Arg : CommonArgProperties<ConfigT>, ChoiceArgProperties, PositionalArgProperties {
            Arg() {}
        ...
        friend void addOptName( Arg& arg, std::string const& optName )
        {
            ...
            if( Detail::startsWith( optName, "--" ) ) {
                ...
                throw std::logic_error( "choice must begin with - or --. Choice was: '" + optName + "'" );
        }
    ...
    public:
        ...
        OptBuilder operator[]( std::string const& optName ) {
            m_choices.push_back( Arg() );
            addOptName( m_choices.back(), optName );
            OptBuilder builder( &m_choices.back() );
            return builder;
        }
        ...
        void optUsage( std::ostream& os, std::size_t indent = 0, std::size_t width = Detail::consoleWidth ) const {
            typename std::vector<Arg>::const_iterator itBegin = m_choices.begin(), itEnd = m_choices.end(), it;
        ...
        std::vector<Parser::Token> populate( std::vector<Parser::Token> const& tokens, ConfigT& config ) const {
            validate();
            std::vector<Parser::Token> unusedTokens = populateChoices( tokens, config );
            unusedTokens = populateFixedArgs( unusedTokens, config );
            unusedTokens = populateFloatingArgs( unusedTokens, config );
            return unusedTokens;
        }

        std::vector<Parser::Token> populateChoices( std::vector<Parser::Token> const& tokens, ConfigT& config ) const {
            std::vector<Parser::Token> unusedTokens;
            std::vector<std::string> errors;
            for( std::size_t i = 0; i < tokens.size(); ++i ) {
                Parser::Token const& token = tokens[i];
                typename std::vector<Arg>::const_iterator it = m_choices.begin(), itEnd = m_choices.end();
                for(; it != itEnd; ++it ) {
                    Arg const& arg = *it;
                        ...
                        if( ( token.type == Parser::Token::ShortOpt && arg.hasShortName( token.data ) ) ||
                            ...
                            if( arg.takesArg() ) {
                                if( i == tokens.size()-1 || tokens[i+1].type != Parser::Token::Positional )
                                    errors.push_back( "Expected argument to choice: " + token.data );
                                else
                ...
                if( it == itEnd ) {
                    if( token.type == Parser::Token::Positional || !m_throwOnUnrecognisedTokens )
                        ...
                        errors.push_back( "unrecognised choice: " + token.data );
                }
            }
            if( !errors.empty() ) {
                std::ostringstream oss;
        ...
        void validate() const
        {
            if( m_choices.empty() && m_positionalArgs.empty() && !m_floatingArg.get() )
                throw std::logic_error( "No choices or arguments specified" );

            for( typename std::vector<Arg>::const_iterator  it = m_choices.begin(),
                                                            itEnd = m_choices.end();
                    it != itEnd; ++it )
                it->validate();
        }
...
namespace Catch
{
    ...
    template<typename T>
    struct LazyStat : Choice<T> {
        LazyStat() : used( false ) {}
        LazyStat& operator=( T const& _value ) {
            Choice<T>::operator=( _value );
            used = false;
            return *this;
        }
        void reset() {
...
namespace Catch {
    ...
    inline Choice<std::size_t> list( Config const& config ) {
        Choice<std::size_t> listedCount;
        if( config.listTests() )
...
namespace Catch {
    ...
    public:
        ...
        struct OnUnusedChoices { enum DoWhat { Ignore, Fail }; };
        ...
        int applyCommandLine( int argc, char const* const* const argv, OnUnusedChoices::DoWhat unusedChoiceBehaviour = OnUnusedChoices::Fail ) {
            try {
                m_cli.setThrowOnUnrecognisedTokens( unusedChoiceBehaviour == OnUnusedChoices::Fail );
        ...
        int run() {
            ...
            try
            {
                ...
                // Handle list request
                if( Choice<std::size_t> listed = list( config() ) )
...
namespace Catch {
    ...
    public:
        ...
        virtual Choice<TagAlias> find( std::string const& alias ) const;
...
namespace Catch {
    ...
    Choice<TagAlias> TagAliasRegistry::find( std::string const& alias ) const {
        std::map<std::string, TagAlias>::const_iterator it = m_registry.find( alias );
        if( it != m_registry.end() )
            return it->second;
        else
            return Choice<TagAlias>();
    }
...
namespace Catch {
    ...
    struct StreamingReporterBase : SharedImpl<IStreamingReporter> {
        ...
        virtual void skipTest( TestCaseInfo const& ) CATCH_OVERRIDE {
            ...
            // It can choiceally be overridden in the derived class.
        }
